...
public class Food {
    ...
    public Food(Rect background, Snake snake, int width, int height, Color color) {
        ...
        yPadding = (int) ((Constants.TILE_HEIGHT - this.height) / 2.0);
    }

    public void spawn() {
        do {
            double randX = (int) (Math.random() * (int) (background.width / Constants.TILE_WIDTH)) * Constants.TILE_WIDTH + background.x;
            double randY = (int) (Math.random() * (int) (background.height / Constants.TILE_HEIGHT)) * Constants.TILE_HEIGHT + background.y;
    ...
    public void draw(Graphics2D g2) {
        g2.setColor(color);
        g2.fillRect((int)this.rect.x + xPadding, (int)this.rect.y + yPadding, width, height);
    }
}
