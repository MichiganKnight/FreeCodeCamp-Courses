...
* To be idiomatic, we use the condition `if value is None:`. If we express `if value:`, a zero length string, the number
  zero, or `None` would all evaluate to `False
...
* `while` - Executes a block of code repeatedly until a condition is `False`
    ...
    * `while False:` - Doesn't execute the block of code
* `for` - Executes a block of code for each item in a collection
    * `range` - Represents an immutable sequence of numbers
    * `range(stop)` - `stop` - Produces a sequence of numbers from 0 to `stop`
    * `range(start, stop, step)`
        * `start` - Initial Value, Inclusive (Includes the Value)
        * `stop` - Iterate by `step` until we reach `stop`, exclusive
